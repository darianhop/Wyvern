...
async def dectalk(client,message):
    try:
        os.chdir(os.path.dirname(os.path.realpath(__file__))+'\\audio')
        subprocess.Popen("say.exe -w "+count+".wav "+msg+"&")
        vc = client.join_voice_channel(message.author.guild.get_channel(VCID))
        player = vc.create_ffmpeg_player("bot\\audio\\"+count+'.wav', after=lambda: print('done playing ',count,'.wav'))
        try:
            player.start()
