        ...
        async def member_list_Sync(self,guild_ID,MEMBER_ROLE_ID):
            ...
            for lines in discObject:
                ...
                try:
                    ...
                    if ((list(filter(lambda person: (person['First'] == name[0])
                            and person['Last'] == name[1], sheets_member_Object)))
                            and lines[1] == 1):
                        user_Mark = next(item for item in internal_member_Object if item['First'] == name[0] and item['Last'] == name[1])
                        user_Mark['Rolled in Discord'] = 'TRUE'
                        user_Mark1 = next(item for item in sheets_member_Object if item['First'] == name[0] and item['Last'] == name[1])
                    ...
                    elif ((list(filter(lambda person: (person['First'] == name[0])
                            and person['Last'] == name[1], sheets_member_Object)))
                            and lines[1] == 0):
                        user_Mark2 = next(item for item in internal_member_Object if item['First'] == name[0] and item['Last'] == name[1])
                        user_Mark2['Rolled in Discord'] = 'FALSE'
                        user_Mark3 = next(item for item in sheets_member_Object if item['First'] == name[0] and item['Last'] == name[1])
